...
class Kohut_SNMP_Printer extends Kohut_SNMP_Abstract
{
    ...
    const SNMP_SUB_UNIT_TYPE_SLOT_5                   = '.1.3.6.1.2.1.43.8.2.1.15.1.3';
    ...
    public function getNumberOfPrintedPapers()
    {
        ...
        $numberOfPrintedPapers = $this->get1(self::SNMP_NUMBER_OF_PRINTED_PAPERS);
        snmp_set_quick_print(false);

        return ($numberOfPrintedPapers !== false) ? (int) $numberOfPrintedPapers : "";
    }
...
public function getBlackCatridgeType2()
    {
        if ($this->isColorPrinter()) {
            return $this->getSNMPString(self::SNMP_SUB_UNIT_TYPE_SLOT_4);
        } elseif ($this->isMonoPrinter()) {
            return $this->getSNMPString(self::SNMP_SUB_UNIT_TYPE_SLOT_5);
        } else {
            return false;
        }
    }
